package com.playground.docs.infrastructure.ingestion.redis;
 ...
 * §A08.2 + ADR-13 §5. The lock-key namespace
 * ({@code docs:lock:} — renamed from {@code rag-ingestion:lock:} in M6.1 per
 * ADR-08 §A08.2) is prepended here so the unqualified key the caller passes
 * (e.g., {@code "document:<uuid>"}) doesn't collide with any other BC's
 * Redis keys.
 *
 * <p>Failure modes:
 * <ul>
...
public class RedissonDistributedLockAdapter implements DistributedLockPort {
    ...
    /** Per-BC lock-key namespace pinned by ADR-08 §A08.2 (renamed from
     *  {@code rag-ingestion:lock:} in M6.1 when the rag-ingestion BC was
     *  absorbed into docs). */
