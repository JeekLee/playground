...
public class ChunkRepositoryJdbcAdapter implements ChunkRepository {
    ...
    public Optional<BodyChecksum> findBodyChecksum(DocumentId documentId) {
        try {
            String value = jdbc.queryForObject(
                    "SELECT body_checksum FROM docs.document_chunks "
                            + "WHERE document_id = ? "
                            + "LIMIT 1",
                    String.class,
                    documentId.value());
    ...
    public void replaceAll(DocumentId documentId, List<DocumentChunk> chunks) {
        ...
        jdbc.update(
                "DELETE FROM docs.document_chunks WHERE document_id = ?",
                documentId.value());
    ...
    public int countByDocument(DocumentId documentId) {
        Integer count = jdbc.queryForObject(
                "SELECT COUNT(*) FROM docs.document_chunks WHERE document_id = ?",
                Integer.class,
                documentId.value());
    ...
    public Optional<ChunkOwnerMeta> findOwnerMeta(DocumentId documentId) {
        try {
            return Optional.ofNullable(jdbc.queryForObject(
                    "SELECT user_id, visibility FROM docs.document_chunks "
    ...
    public List<UUID> findAllDistinctDocumentIds() {
        return jdbc.query(
                "SELECT DISTINCT document_id FROM docs.document_chunks ORDER BY document_id",
    ...
    public List<UUID> findDistinctDocumentIdsByUser(UUID userId) {
        return jdbc.query(
                "SELECT DISTINCT document_id FROM docs.document_chunks WHERE user_id = ? ORDER BY document_id",
                (rs, i) -> (UUID) rs.getObject("document_id"),
                userId);
    }
}
