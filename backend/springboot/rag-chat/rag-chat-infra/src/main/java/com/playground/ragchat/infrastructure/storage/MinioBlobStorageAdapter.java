...
/**
 * MinIO-backed {@link BlobStoragePort} for rag-chat attachment downloads per ADR-20 §D3 revised.
 *
 ...
 * artifact envelope. rag-chat only needs {@link #get} to serve downloads; the
...
public class MinioBlobStorageAdapter implements BlobStoragePort {
    ...
    public MinioBlobStorageAdapter(
            MinioClient client,
            @Value("${PLAYGROUND_RAGCHAT_MINIO_BUCKET:rag-chat-attachments}") String bucket) {
        this.client = client;
        this.bucket = bucket;
    }
