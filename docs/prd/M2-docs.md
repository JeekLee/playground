...
- [ ] M2 P0에서 저장은 **편집 멈춤 1.5초 debounce auto-save + `⌘+S` / `Ctrl+S` 즉시 저장** 두 경로를 모두 지원한다 (design doc §"New document (editor)" / §"Edit document"). 저장 상태는 에디터 툴바 좌측 save-state pill에 `Saving…` / `Saved 3s ago` / `Save failed — retry`로 표시된다 (별도 Save 버튼은 없다 — 디자인 의도). spec §2의 "auto-save deferred" 문구는 v5 design doc의 "⌘+S or autosave" 결정에 의해 무효화되었다.
