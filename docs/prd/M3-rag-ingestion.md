...
- [x] **M3.1 amendment (2026-05-19):** `heading_path text[] NOT NULL DEFAULT '{}'` 컬럼이 추가된다 (Flyway `V202605200003__add_chunk_heading_path.sql`). 새 ingest는 markdown-aware chunker가 계산한 헤딩 브레드크럼 배열을 채우고, pre-migration row는 빈 배열을 carry. M4 citation UI가 향후 이 컬럼을 활용 가능.
...
- [x] **Re-embedding job (shipped 2026-05-19):** 청킹 파라미터 또는 임베딩 모델이 바뀌었을 때 전체 corpus 재임베딩 — `reembed` Spring profile + `ReembedCommandLineRunner`로 구현. Scope=`all`/`user`/`document`, 동일 Redisson 락 직렬화, 실패 시 exit code 2. ADR-13 §7 참조.
- [x] **Markdown-aware chunker + `heading_path` 컬럼 (shipped 2026-05-19):** ADR-13 §1 alternative (c)에서 deferred됐던 의미 단위 청커. commonmark-java AST → section build → window normalize. 펜스/테이블 atomic, oversize paragraph는 sentence-split, heading-aware breadcrumb prefix. 기존 token-window 알고리즘은 parse-fallback 경로로 보존.
...
- ~~Adaptive chunk size (semantic chunking, layout-aware)~~ — M3.1에서 cover됨 (markdown-aware chunker 항목 참조).
