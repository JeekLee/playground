...
- [ ] M4가 다음 단일 SQL 형태로 corpus를 분리할 수 있다 (M3가 만들어주는 invariant):
  - 익명: `SELECT ... FROM rag.document_chunks WHERE visibility = 'public' ORDER BY embedding <=> :q LIMIT :k`
...
- [ ] Manual E2E: M2에서 공개 문서 업로드 → M3 chunk가 `visibility='public'`으로 저장됨 → M4(미래)의 익명 chat에서 그 문서가 인용된 답변. M3 close blocker 아님.
