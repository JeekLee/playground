...
- **Interactions:**
  - Click any Stop button → SSE `close()` on the client. **Server pipeline keeps running** to completion so the assistant message + citations persist (revised 2026-05-18 — was "not persisted"; see M4 spec §6.1 step 12). On the next visit the persisted assistant turn shows up via `GET /sessions/{id}/messages`.
  - Composer is non-interactive (`pointer-events: none`, `opacity: 0.6` per design system §6.2 disabled-input convention).
  - Tab switch during stream → SSE closes on the client; server pipeline continues. Returning to the tab triggers a **mid-stream re-join** (spec §6.4): the page does `GET /api/rag/chat/sessions/{id}/stream`, the in-memory `ActiveTurnRegistry` returns the still-running replay-shared flux, and the streaming bubble fills with the buffered `retrieval` + every token emitted so far + the live tail (or the final `done` if the turn finished while away). 404 → caller renders pure history. No UI surface added — same mid-stream visual as `54:180`.
- **Per-state:**
...
- **Card list (when expanded):** 820×auto frame, `surface` bg + `border` 1px + `radius.md` 10px. Cards stacked vertically with 1px `border` dividers between them.
  ...
  - **No citations (N=0):** the accordion is **not rendered at all** — neither the streaming row nor a loaded-history row mounts it when `citations.length === 0`. An unsupported / retrieval-empty answer conveys that in its own body text (e.g. `참고할 수 있는 내용이 없습니다…`); there is no empty "Citations · none" card. _(Revised 2026-06-08: the streaming row previously mounted an auto-opened "(no citations — answer was unsupported)" card on the terminal `done` event, which then vanished the instant the row became history — a visible flicker. Both rows now gate on `citations.length > 0`. The `RETRIEVAL_EMPTY` SSE error code remains defined but is not emitted by the backend.)_
...
- **Inline `[N]` marker pill (open question #1):** rendered as a `min-width: 18px` button with `accent.soft` bg, `accent` fg, font-mono 10/600, `radius.pill`, ~2 px x-padding. Hover swaps to `accent` bg + `surface` fg for affordance. Clicking scrolls + auto-expands the matching accordion card and applies a brief 2 s **inset 2px `accent` ring** on the card row (revised 2026-05-20 — was `accent.soft` bg fill, which collided with the inline pill's own `accent.soft` background and read as a flat block rather than a focus indicator). Orphan markers (no matching citation row) render as plain `[N]` text per ADR-14 §10 / spec §11 Q7.
