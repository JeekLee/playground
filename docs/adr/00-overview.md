...
| 08 | `08-inter-service-comms.md` | Gateway-to-BC HTTP, BC-to-BC Kafka only. **Amendments:** M2 (ADR-12, Exception 3 docs→identity), M3 (ADR-13, informational only), M6.1 (ADR-12 §A08.1–§A08.7 — Exception 1 retired, Exception 2 namespace renamed, MinIO direction §A08.3), **M7 (ADR-17 §A08.8 — Exception 4 introduced for rag-chat → tool BCs HTTP; template with per-tool-BC sub-row)** |
...
modules** + `buildSrc`. Port 18083 returns to the reservation pool. The
runnable port table:
...
- ~~rag-ingestion-api 18083~~ — **retired**; port reserved
- rag-chat-api 18084
- (reserved) 18083, 18085
- metrics-api 18086
